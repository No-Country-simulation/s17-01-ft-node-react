...
export function PricingCard(props:PricingCardProps) {
    return (
        <div className={`${styles.container} ${props.recommended && styles.recommended}`}>
                ...
                {props.benefits.map((b, index) => {
                    return (
                        <li key={b} className={styles.benefit_item}>
                            <Check color="var(--color-primary)" size={20}/><p>{b}</p>
                        </li>
                )
                })}
            </ul>
            <Button onClick={props.onClick} variant={props.button.variant}>{props.button.text}</Button>
        </div>
    );
};
