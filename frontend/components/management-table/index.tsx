...
function ComponentItem(props:ComponentItemProps) {
    ...
    return (
        <div onClick={()=>setChecked(!checked)} className={styles.item_container}>
            <input type="checkbox" checked={checked} className={styles.item_checkbox}/>
            <b className={styles.item_title}>{props.title}</b>
            <p className={styles.item_date}><span>Fecha: </span>{format(props.date, "dd-mm-yyyy").replaceAll("-", "/")}</p>
            <p className={styles.item_views}><span>Vistas: </span>{parseViews(props.views)}</p>
