...
export class ComponentsService {
  ...
  async create(createComponentDto: CreateComponentDto): Promise<Component> {
    ...
    if (uploader && !(await this.userRepository.findOne({ where: { id: uploader.id } }))) {
      throw new BadRequestException(`Uploader with ID ${uploader.id} not found`);
    }
    if (categories) {
      for (const category of categories) {
        if (!(await this.categoryRepository.findOne({ where: { id: category.id } }))) {
          throw new BadRequestException(`Category with ID ${category.id} not found`);
        }
      }
    }
    if (plan && !(await this.planRepository.findOne({ where: { id: plan.id } }))) {
  ...
  async findByCategory(categories: number): Promise<Component[]> {
    ...
    if (components.length === 0) {
      throw new NotFoundException(`No components found for category with ID ${categories}`);
    }

    return components;
  }

  async findByPlan(plan: number): Promise<Component[]> {
    const components = await this.componentRepository
      .createQueryBuilder('component')
    ...
    if (components.length === 0) {
      throw new NotFoundException(`No components found for plan with ID ${plan}`);
    }
  
    return components;
  }

  async findByCategoryAndPlan(categories: number, plan: number): Promise<Component[]> {
    const components = await this.componentRepository
      .createQueryBuilder('component')
      .innerJoinAndSelect('component.categories', 'category')
      .innerJoinAndSelect('component.plan', 'plan')
    ...
    if (components.length === 0) {
      throw new NotFoundException(`No components found for category with ID ${categories} and plan with ID ${plan}`);
    }
  ...
  async findByTier(tier: number): Promise<Component[]>{
    const components = await this.componentRepository
      ...
      .getMany();
      if (components.length === 0) {
        throw new NotFoundException(`No components found for tier with ID ${tier}`);
      }
  ...
  async findOne(id: number): Promise<Component> {
    ...
    .createQueryBuilder('component')
    .leftJoinAndSelect('component.uploader', 'uploader')
    .leftJoinAndSelect('component.categories', 'categories')
    .leftJoinAndSelect('component.plan', 'plan')
